package Api::Admin::Controllers::Authorization;
...
sub auth($self) {
  ...
  my $authorization_admin_use_case = $self->app->{container}->get('authorization_admin_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $either = $authorization_admin_use_case->auth({token => $access_token});
    ...
    if ( $either->is_right() ) {
      $logger_adapter->info({
        message => 'Successfully authorization admin'
      });
