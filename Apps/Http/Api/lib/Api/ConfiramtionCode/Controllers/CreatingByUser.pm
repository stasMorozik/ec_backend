package Api::ConfiramtionCode::Controllers::CreatingByUser;
...
sub create($self) {
  ...
  my $creating_by_user_confirmation_code_use_case = $self->app->{container}->get('creating_by_user_confirmation_code_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $either = $creating_by_user_confirmation_code_use_case->create($self->req->json);
