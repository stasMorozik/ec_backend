package Api::ConfiramtionCode::Controllers::ConfirmingByUser;
...
sub confirm($self) {
  ...
  my $confirming_by_user_confirmation_code_use_case = $self->app->{container}->get('confirming_by_user_confirmation_code_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $either = $confirming_by_user_confirmation_code_use_case->confirm($self->req->json);
