package Api::Token::Controllers::RefreshingByUser;
...
sub refresh($self) {
  ...
  my $refreshing_by_user_token_use_case = $self->app->{container}->get('refreshing_by_user_token_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $either = $refreshing_by_user_token_use_case->refresh({token => $refresh_token});
