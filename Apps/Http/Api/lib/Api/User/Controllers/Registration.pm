package Api::User::Controllers::Registration;
...
sub registry($self) {
  ...
  my $registration_user_use_case = $self->app->{container}->get('registration_user_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $args = $self->req->json;
    
    my $either = $registration_user_use_case->registry({
      email => $args->{email},
      name => $args->{name},
      ...
      role => Core::User::Validators::Role::user()
    });
