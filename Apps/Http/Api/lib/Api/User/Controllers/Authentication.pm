package Api::User::Controllers::Authentication;
...
sub auth($self) {
  ...
  my $authentication_user_use_case = $self->app->{container}->get('authentication_user_use_case');
  my $logger_adapter = $self->app->{container}->get('logger_adapter');

  try {
    my $either = $authentication_user_use_case->auth($self->req->json);
    ...
    if ( $either->is_right() ) {
      $logger_adapter->info({
        message => 'Successfully authentication user'
      });
